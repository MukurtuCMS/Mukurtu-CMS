  ...
  public function buildForm(array $form, FormStateInterface $form_state, string $entity_type = '', string $entity_id = '') {
    ...
    if ($entity->getEntityTypeId() === 'node' && in_array($entity->bundle(), ['collection', 'word_list'])) {
      ...
      if ($child_count > 0) {
        ...
        $item_count = $entity->bundle() === 'collection' && $entity->hasField('field_items_in_collection')
          ? count($entity->get('field_items_in_collection')->referencedEntities())
          : $child_count;
        $form['include_children'] = [
          '#type' => 'checkbox',
          '#title' => $entity->bundle() === 'word_list'
            ? $this->formatPlural(
                $child_count,
