...
trait TraitDescriptions {
  ...
  public function baseDescriptions(): array {
    ...
    return [
      ...
      'preload' => $this->t("Preload to optimize the loading of late-discovered resources (CSS backgrounds), or critical resources (hero images). By preloading a resource, you tell the browser to fetch it sooner than the browser would otherwise discover it before Native lazy or lazyloader JavaScript kicks in, or starts its own preload or decoding. The browser caches preloaded resources so they are available immediately when needed. Nothing is loaded or executed at preloading stage. <br><br>To avoid potential overuse, this is limited to heroe images defined by <b>Loading priority: unlazy and slider</b>, <a href=':url'>read more</a>.", [
